...
class Tello:
    ...
    def __init__(self,
        ...
        try:
            ...
            temp_sock.connect((host, 8889))
            local_ip = temp_sock.getsockname()[0]
            temp_sock.close()
            # If the detected IP is valid (not 0.0.0.0), use it for video stream
            # if local_ip != '0.0.0.0':
            #     self.VS_UDP_IP = local_ip
        except Exception as e:
            print(f"Could not detect local IP, falling back to 0.0.0.0: {e}")
...
class BackgroundFrameRead:
    ...
    def __init__(self, tello, address, with_queue = False, maxsize = 32):
        ...
        # Try grabbing frame with OpenCV
        try:
            ...
            raise TelloException(f'Failed to grab video frames from video stream: {e}')
    ...
    def update_frame(self):
        """Thread worker function to retrieve frames using OpenCV
        Internal method, you normally wouldn't call this yourself.
        """
        while not self.stopped:
            ret, frame = self.cap.read()
            if not ret:
                # If we fail to read a frame, we might want to wait a bit and try again, 
                # or break depending on the behavior we want. For now, let's wait a bit.
                time.sleep(0.01)
                continue

            # Convert BGR (OpenCV) to RGB (what the rest of the code expects)
